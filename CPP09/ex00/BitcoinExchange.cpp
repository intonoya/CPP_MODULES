...
BitcoinExchange::BitcoinExchange(std::ifstream& dataFile)
{
    ...
    while (std::getline(dataFile, eachLine))
    {
        ...
        {
            _data.clear();
            std::cout<<"\33[1;31mError: bad input:  \33[1;31m"<<date<<std::endl;
            return ;
        }
        ...
        {
            _data.clear();
            std::cout<<"\33[1;31m[data.csv] \33[1;31m"<<rateAndText.first;
            return ;
        }
...
std::pair<std::string, float>   BitcoinExchange::_checkValue(const std::string& valueStr)
{
    if (valueStr == "" || valueStr == "+" || valueStr == "-")
        return std::make_pair("\33[1;31mError: missing value: \33[1;31m", 0.0);
    ...
    if (forbiddenChars == std::string::npos)
    {
        ...
        if (integerValue > INT_MAX)
            return std::make_pair("\33[1;31mError: the number is too large!\n\33[1;31m", 0.0);
        if (integerValue < INT_MIN)
            return std::make_pair("\33[1;31mError: the number is too small!\n\33[1;31m", 0.0);
        if (integerValue < 0)
            return std::make_pair("\33[1;31mError: the number is not positive!\n\33[0m", 0.0);
        return std::make_pair("valid", static_cast<float>(integerValue));
    }
    char*   end;
    float   floatValue = strtof(valueStr.c_str(), &end);
    if (*end != '\0')
        return std::make_pair("\33[1;31mError: the number is not valid!\n\33[1;31m", 0.0);
    ...
    while(std::getline(inputFile, eachLine))
    {
        ...
        if (datePart)
        {
            ...
            while (--datePart && datePart >= startIt->first && it == _data.end())
                it = _data.find(datePart);
            if (it == _data.end())
                std::cout<<"\33[1;31mError: there is no such date in data.csv: \33[1;31m"<<date<<std::endl;
            else
            {
                ...
                if (valueAndText.first != "valid")
                {
                    ...
                    std::cout<<"\33[1;31mError: the price is greater than 1000: \33[1;31m"<<value<<std::endl;
                else
                {
                    std::cout<<"\33[1;32m"<<date<<"\33[1;32m -> \33[1;32m"<<valueAndText.second<<" = ";
                    std::cout<<valueAndText.second * (*it).second<<std::endl;
                }
            }
        }
        else
            std::cout<<"\33[1;31mError: bad input -> \33[1;31m"<<date<<"\033[0m"<<std::endl;
    }
}

void BitcoinExchange::_checkInput(const std::string &filename)
{
    ...
    {
		file.close();
		throw std::runtime_error("\33[1;31mError: Invalid file format!\33[1;31m");
	}
	file.close();
}